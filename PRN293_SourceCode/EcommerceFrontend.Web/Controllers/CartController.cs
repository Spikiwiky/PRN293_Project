    ...
    public class CartController : ControllerBase
    {
        ...
        public async Task<IActionResult> TestCartSimple()
        {
            try
            {
                ...
                // Now test cart
                var cartUrl = $"{backendUrl}/api/Cart";
                
                // Forward cookies to backend
                var cartRequest = new HttpRequestMessage(HttpMethod.Get, cartUrl);
                var cookieHeader = Request.Headers["Cookie"].ToString();
                if (!string.IsNullOrEmpty(cookieHeader))
                {
                    cartRequest.Headers.Add("Cookie", cookieHeader);
                }
                else
                {
                    // Fallback: manually add UserId cookie
                    cartRequest.Headers.Add("Cookie", $"UserId={userId}");
                }
        ...
        public IActionResult DebugCart()
        {
            try
            {
                _logger.LogInformation("=== DebugCart called ===");
            ...
            {
                _logger.LogError(ex, "Error in DebugCart");
                return StatusCode(500, new { success = false, error = ex.Message });
            }
        }
        ...
        public async Task<IActionResult> GetCartSummary()
        {
            try
            {
                ...
                var summaryUrl = $"{backendUrl}/api/Cart/summary";
                _logger.LogInformation("Calling summary URL: {SummaryUrl}", summaryUrl);
                
                // Forward cookies to backend
                var httpRequest = new HttpRequestMessage(HttpMethod.Get, summaryUrl);
                var cookieHeader = Request.Headers["Cookie"].ToString();
                if (!string.IsNullOrEmpty(cookieHeader))
                {
                    httpRequest.Headers.Add("Cookie", cookieHeader);
                ...
                {
                    // Fallback: manually add UserId cookie
                    httpRequest.Headers.Add("Cookie", $"UserId={userId}");
                }
